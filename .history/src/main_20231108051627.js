...
function game_loop(curr_time) {
	...
	let delta_ms = curr_time - prev.frame;
	if (delta_ms >= frame_interval) {
		prev.frame = curr_time - (delta_frame_ms % frame_interval)

		delta_ms = curr_time - prev.fall;
		if (delta_ms > tetris.fall_interval) {
			prev.fall = curr_time - (delta_fall_ms % tetris.fall_interval);
			console.log(prev);
