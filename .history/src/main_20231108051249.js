...
function game_loop(curr_time) {
	...
	if (delta_frame_ms >= frame_interval) {
		...
		prev.frame = curr_time;

		const delta_fall_ms = curr_time - prev.fall_interval;
		if (delta_fall_ms > tetris.fall_interval) {
			prev.fall_back = curr_time - (delta_fall_ms % tetris.fall_interval);
