...
export class Tetronimo {
	...
	fall(tetris, curr_time, prev_fall_tick) {
		let delta_ms = curr_time - prev_fall_tick;
		// only fall if enough time has elapsed
		if (delta_ms < tetris.fall_tick)
			return true;

		prev_fall_tick = curr_time - (delta_ms % tetris.fall_tick)
	...
	hard_drop(tetris, curr_time, delta_fall_tick_ms) {
