...
export class Tetronimo {
		...
		switch (key) {
			...
			case KEY.DOWN:
				if (this.max_y() < PLAYFIELD_YMAX - 1 && this.check_move(grid, 0, +1)) {
					this.origin[1]++;
					score += 1; // This will add 1 extra point for every block hard/soft dropped.
				}
				break;
			case KEY.RIGHT:
	...
	hard_drop(grid, score) {
		while (this.fall(grid)) {
			this.move(KEY.DOWN, grid, score);
		}
	}
}
...
export class GFX {
	...
	draw_ui_score(score) {
		...
		let y = 500;
		this.draw_ui_text(`Score: ${score.toString().padStart(10, '0')}`, 30, x, y);
	}
}
